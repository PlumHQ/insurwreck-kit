...
- `site/` — the public attendee event site, deployed as the separate `insurwreck-4` project at `https://insurwreck-4.preview.plumhq.com` (`/` attendee page and `/deck` orientation walkthrough, both behind Plum-only Google sign-in; `/concept` brand exploration). `site/DESIGN.md` is the source of truth for its visual system and the copy voice used across event communication.
