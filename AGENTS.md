...
## 1. Never deploy the desk by hand

The Vercel project is git-connected: `PlumHQ/insurwreck-kit` → branch `main` → Root Directory `desk`. **Pushing to `main` is the deploy.**
...
- To see what shipped: `vercel ls insurwreck-desk --scope plum` (git deploys carry a commit SHA; manual ones do not, which is why they are untraceable).
...
This repo goes public before the event. Master keys live only in the Vercel environment (`vercel env ls production --scope plum`) and are documented **by name only** in `desk/.env.example`. Local values live in `.env.local`, which is gitignored.
