...
Kula, Salesforce and Keka ship with the plugin and start automatically. Don't run
`claude mcp add` for these three; sending someone to a second copy of the same server is
how you end up with two entries and a confused participant. Send them to the command:
...
| Keka (their own HR record) | `/insurwreck:keka-connect` | Yes - per-user OAuth, Keka enforces their permissions |

Because these three are always on, they cost context on every turn for everyone. If a
