...
2. From the file, show:
   ...
   - A status table for Vercel, Supabase, n8n, Resend, AgentMail: **Ready** if present under `services`, **Almost ready** if the entry has `"incomplete": true` (name its `pending_parts`), **Pending** if listed under `pending`.
