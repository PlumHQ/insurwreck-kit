...
1. Run `iw-status`. It reads `~/.insurwreck/credentials.json` and prints the table in a fixed format — show its output as-is rather than re-formatting it, so the layout is the same for everyone every time. It never prints a secret. If it reports no setup found, tell them to run `/insurwreck:start` and stop. If `iw-status` is missing, fall back to reading the bundle yourself.
