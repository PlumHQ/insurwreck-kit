...
Salesforce is the one MCP server that needs a per-participant login and doesn't have its own command. Keka has one — `/insurwreck:keka-connect`. Kula needs nothing; it's covered below for when someone asks.
