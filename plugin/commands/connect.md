...
- **Check `services.clevertap.access_level`.** `campaign` means they can also create and stop campaigns; `full` means they additionally have profile deletion, consent changes and `clevertap_request`. Either way, `services.clevertap.campaign_note` spells out the consequences and is worth reading to them verbatim.
- **At either level**, they can create and stop campaigns. Say what that means before they use it: `create_campaign` with `when: "now"` sends real push, email or SMS to real Plum members immediately, with no recall. Tell them to schedule rather than send-now while building, and to test against a segment of one before anything wider. `services.clevertap.campaign_note` says the same thing.
